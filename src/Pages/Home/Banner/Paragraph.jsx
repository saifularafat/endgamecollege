...
const Paragraph = ({ title, colorTitle, subTitle, label, address, labelOne, addressOne }) => {
    return (
        <div className=' text-white text-center w-5/5 mx-auto md:space-y-4 relative items-center lg:left-1/3 md:left-16 overflow-hidden'>
            ...
            <div className='flex items-center justify-center gap-3'>
                <Link to={address} className='banner_btn'>
                    {label}
                </Link>
                <Link to={addressOne} className='banner_btn_white'>
                    {labelOne}
                </Link>
            </div>
        </div>
